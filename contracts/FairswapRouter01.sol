...
import './interfaces/IFairswapRouter01.sol';
import './interfaces/IERC20.sol';
import './interfaces/IWETH.sol';

contract FairswapRouter01 is IFairswapRouter01 {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = FairswapLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = FairswapLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'FairswapRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = FairswapLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function removeLiquidity(
        ...
        address pair = FairswapLibrary.pairFor(factory, tokenA, tokenB);
        IFairswapPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
    ...
    function removeLiquidityWithPermit(
        ...
        IFairswapPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            IFairswapPair(FairswapLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, FairswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) public view override returns (uint amountOut) {
        return FairswapLibrary.getAmountOut(factory, amountIn, reserveIn, reserveOut);
    }

    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public view override returns (uint amountIn) {
        return FairswapLibrary.getAmountOut(factory, amountOut, reserveIn, reserveOut);
    }

    function getAmountsOut(uint amountIn, address[] memory path) public view override returns (uint[] memory amounts) {
        return FairswapLibrary.getAmountsOut(factory, amountIn, path);
    }

    function getAmountsIn(uint amountOut, address[] memory path) public view override returns (uint[] memory amounts) {
        return FairswapLibrary.getAmountsIn(factory, amountOut, path);
    }
}
